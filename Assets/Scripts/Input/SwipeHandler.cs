...
namespace SwipeMenu
{
	...
	public class SwipeHandler : MonoBehaviour
	{
		...
		public bool handleSwitchMenu = true;
		...
		public static SwipeHandler Instance{ get; set; }
...
//#endif
        ...
        {
			if (EventSystem.current.IsPointerOverGameObject() || !handleSwitchMenu)
            {
                return;
            }
		...
		private void HandleMouseSwipe ()
		{
			...
			if (Input.GetMouseButtonUp (0)) {
				...
				endpos = new Vector3 (touchPosition.x, 0, touchPosition.y);
				var finalPositionX = endpos.x - startpos.x;
                //length = finalPosition.x < 0 ? (finalPosition.magnitude * Time.deltaTime) : -(finalPosition.magnitude * Time.deltaTime);
                length = finalPosition.x < 0 ? (finalPositionX * Time.deltaTime) : -(finalPositionX * Time.deltaTime);
                length *= .5f;
