...
namespace SwipeMenu
{
    ...
    public class Menu : MonoBehaviour
    {
        ...
        public Toggle[] navigationButtons;
...
//		public void AnimateToTargetItem(MenuItem item, bool isActive ){
//			if (isActive) {
...
//		}
		...
		public void AnimateToTargetItem(MenuItem item)
        {
            ...
            if (!_isAnimating)
			{	
				var index = Array.IndexOf (menuItems, item);
				if (index == 0) {
					nav.MoveToTargetLeft (true);
				} else if (index == 1) {
					nav.MoveToTargetCenter (true);
				} else {
					nav.MoveToTargetRight (true);
				}
                _isAnimating = true;
                ...
                iTween.ValueTo(gameObject, iTween.Hash("from", _currentMenuPosition, "to", _currentMenuPosition + offset,
                                                         "time", 0.5, "easetype", iTween.EaseType.easeOutCubic,
                                                         "onupdate", "UpdateCurrentMenuPosition", "oncomplete", "AnimateToTargetComplete"));
                navigationButtons[Array.IndexOf(menuItems, item)].isOn = true;
            }
        }

        void AnimateToTargetComplete()
        {
            _isAnimating = false;
        ...
        {
            foreach (var toggle in navigationButtons)
            {
